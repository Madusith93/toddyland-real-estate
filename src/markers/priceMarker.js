...
function formatPriceLabel(property) {
  ...
  const rawPrice = property.price || parseInt((property.priceLabel || '').replace(/[^0-9]/g, '')) || 0
  ...
  const isMonthly = (property.priceLabel || '').includes('/mo')
  return `${symbol}${formatted}${isMonthly ? '/mo' : ''}`
}

const BASE_BG = 'rgba(239, 68, 68, 0.55)'
const HOVER_BG = 'rgba(220, 38, 38, 0.85)'
...
export function createPriceMarkerElement(property, selected = false) {
  ...
  label.style.background = BASE_BG
  ...
  label.style.border = '1.5px solid rgba(220, 38, 38, 0.6)'
  ...
  wrapper.addEventListener('mouseenter', () => {
    ...
    label.style.background = HOVER_BG
