...
function PropertiesContent() {
  ...
  const finalDisplayProperties = filteredProperties.filter((property: any) => {
    if (filters.tab === 'rent' && property.type !== 'rental') return false
    if (filters.tab === 'land' && property.type !== 'land') return false
    if (filters.tab === 'buy' && (property.type === 'rental' || property.type === 'land')) return false
      ...
      initRegionTool((region: any) => {
        if (region) {
          ...
          const regionProperties = propertiesCache.filter((property: any) =>
            property.region?.toLowerCase().includes(region.key?.toLowerCase()) ||
            property.district?.toLowerCase().includes(region.key?.toLowerCase())
          );
